...
$stmt = $pdo->query("SELECT * FROM sponsorship
                    WHERE service = 'Buy Me a Coffee'
                    ORDER BY publication_date DESC
                    LIMIT 1");
$sponsorship_link = $stmt->fetchAll(PDO::FETCH_ASSOC);
foreach ($sponsorship_link as &$r) {
    if (!empty($r['publication_date'])) {
        $r['publication_date'] = date('Y-m-d\TH:i:s', strtotime($r['publication_date']));
    }
}
...
echo json_encode($sponsorship_link);